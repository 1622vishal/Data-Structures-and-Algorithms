...
Node* deletelast(Node* Head)
{
    ...
    { //We can do this ny assuming it as first node and creating a temp pointer but this is the easy way
        free(Head);
        return NULL;
    }
    Node* temp = Head;
    while(temp->next->next != NULL)      //We can do this because everything is pointer here
    {
        temp = temp->next;
    }
    free(temp->next);
