...
/*Program to insert a node before a given node in a doubly linked list*/
...
{
    Node* behind = givennode->prev;
    Node* newnode = new Node(value , givennode , behind);
    behind->next = newnode;
...
int main()
{
    ...
    insertbeforenode(head->next->next , value);      //Never pass head if you do so then head will change
    Node* temp = head;
    while(temp)
    {
        cout<<temp->data<<" ";
